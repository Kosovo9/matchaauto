   ...
   */
    ...
    async updateVehiclePosition(c: Context) {
        try {
            ...
            await this.cache.set(`vehicle:pos:${validated.vehicleId}`, validated.position.lat, validated.position.lng, validated, 60);

            // Async process to DB
            this.service.updateVehiclePosition(validated).catch(err => logger.error('Async pos update failed', err));

            metrics.increment('tracking.position_update');
    ...
    async bulkUpdatePositions(c: Context) {
        try {
            ...
            // Process in batches
            await this.service.bulkInsertPositions(validated.positions);

            metrics.increment('tracking.bulk_update', { count: validated.positions.length });
